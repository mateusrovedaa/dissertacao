...
<body>
    ...
    <script>
        ...
        const EVENT_STYLES = {{
            'backoff': {{ color: '#22c55e', pointStyle: 'circle', label: '⬆ Alg.2 Backoff' }},           // Green - stable count reached
            'backoff_reset': {{ color: '#f59e0b', pointStyle: 'triangle', label: '⬇ Alg.3 Vigilância' }},  // Orange - signal variation detected
            'interval_reset': {{ color: '#dc2626', pointStyle: 'rectRot', label: '⬇ Mudança Risco' }}  // Red - risk level change
        }};
                            ...
                            callbacks: {{
                                label: function(context) {{
                                    ...
                                    if (p.eventType === 'backoff') {{
                                        ...
                                        tooltip = `⬆ Alg.2 Backoff Exponencial: ${{p.oldInterval}}s → ${{p.y}}s (K=${{stableCount}} leituras estáveis)`;
                                    }} else if (p.eventType === 'backoff_reset') {{
                                        const delta = p.extraInfo?.delta?.toFixed(1) || '?';
                                        const epsilon = p.extraInfo?.epsilon || '?';
                                        tooltip = `⬇ Alg.3 Vigilância Contínua: ${{p.oldInterval}}s → ${{p.y}}s (Δ=${{delta}} > ε=${{epsilon}})`;
                                    }} else if (p.eventType === 'interval_reset') {{
                                        const oldRisk = p.extraInfo?.old_risk || '?';
                                        const newRisk = p.extraInfo?.new_risk || '?';
                                        tooltip = `⬇ Mudança de Risco: ${{p.oldInterval}}s → ${{p.y}}s (${{oldRisk}} → ${{newRisk}})`;
                                    }}
                                    
                                    return tooltip + ` | Risco: ${{p.risk}}`;
                                }}
                            }}
                        }}
                    }},
