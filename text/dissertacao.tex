...
%=======================================================================
\begin{abstract}
A Internet das Coisas (IoT) tem impactado de forma expressiva diversas áreas, especialmente a medicina, ao viabilizar o monitoramento contínuo e remoto de pacientes por meio de arquiteturas \textit{Edge-Fog-Cloud}, que combinam conectividade e processamento em tempo hábil. O desafio central está no grande volume de dados gerado por múltiplos sensores, que pode sobrecarregar a infraestrutura de rede, aliado à necessidade de garantir que alterações clínicas críticas sejam comunicadas com latência mínima, permitindo intervenções rápidas. Neste cenário, o tema desta dissertação é a otimização do monitoramento de sinais vitais em ambientes de saúde conectados, com foco na utilização eficiente de recursos computacionais e de rede. Embora existam avanços relevantes na área, a maioria dos trabalhos relacionados trata de compressão e priorização de forma separada, sem integrar uma compressão multissinal adaptativa a uma priorização clínica baseada em escores padronizados. Além disso, falta um ciclo fechado em que a inteligência das camadas \textit{fog-cloud} reconfigure dinamicamente a coleta na camada \textit{edge}. Esta dissertação propõe o modelo \textit{Vital Sign Prioritization and Adaptive Compression} (ViSPAC), que unifica a compressão e a priorização adaptativa com base no escore clínico National Early Warning Score 2 (NEWS2). O escore é utilizado para ajustar, em tempo hábil, os parâmetros de compressão e a frequência de coleta, otimizando os recursos sem comprometer a fidelidade dos sinais. O modelo incorpora uma estratégia de controle que equilibra eficiência e segurança clínica por meio de três algoritmos interdependentes: configuração dinâmica por sinal vital, \textit{back-off} exponencial condicionado ao risco e vigilância contínua (\textit{keep-alive}). Os experimentos foram conduzidos em ambiente distribuído na AWS, com 20 nós de borda operando sob restrições de hardware semelhantes às de dispositivos IoT, 1 nó para \textit{fog} e 1 nó para \textit{cloud}, em redes separadas. Os resultados demonstraram que o ViSPAC reduziu em 96,7\% o número de transmissões em relação à linha de base, alcançou uma taxa de compressão média de 81,6\% (superior aos 75,0\% do cenário estático) e obteve uma distorção PRD média global de apenas 1,16\%, substancialmente inferior aos 3,62\% do modelo estático. A latência média do ciclo de \textit{feedback} foi de 1.048,9 ms, adequada ao monitoramento remoto não invasivo. Em síntese, o ViSPAC apresenta-se como uma solução equilibrada para o monitoramento de saúde em larga escala no contexto de Cidades Inteligentes, otimizando recursos de rede e de energia sem comprometer a capacidade de resposta a eventos clínicos adversos.
\end{abstract}
...
\begin{enumerate}
    \item Como integrar e sincronizar fluxos multissinais em uma arquitetura de compressão adaptativa?
...
No VSAC \cite{Andrade2025}, o parâmetro de desvio (denominado $DC$ ou \textit{desvio}) corresponde à amplitude da janela do algoritmo SDT e é um dos dois parâmetros dinâmicos do módulo de compressão com perdas, ao lado do tempo máximo ($T_m$). Ambos partem de um valor padrão e podem ser ajustados em tempo de execução pelo módulo \textit{SetUp}. Quanto maior o valor de $DC$, maior a tolerância do algoritmo a variações no sinal e, consequentemente, maior o descarte de pontos e a taxa de compressão. Em contrapartida, valores elevados de $DC$ ampliam a distorção do sinal reconstruído, podendo mascarar variações clinicamente relevantes. Valores reduzidos de $DC$ preservam melhor a morfologia do sinal, porém resultam em menor compressão.
...
$\geq$ 7 & Alto & Monitoramento contínuo & Alerta emergencial imediato; avaliação por equipe de resposta rápida; considerar transferência para Unidades de Terapia Intensiva (UTI) \\ \hline
\end{tabular}
...
A sequência de busca foi aplicada às bases de dados apresentadas na Seção \ref{sec:strat} para selecionar os estudos. No total foram encontrados 748 artigos, incluindo todas as bases de dados: IEEE Xplore (259), ACM Digital Library (262), ScienceDirect (166) e Springer Link (61). A busca se delimitou em artigos no idioma inglês publicados à partir de 2023 até maio de 2025. Essa escolha de período se deu por conta da pesquisa já realizada para a construção do VSAC, que se estendeu até 2023. Além disso, foram empregados critérios de exclusão compostos por três filtros (F1, F2 e F3) usados sequencialmente para selecionar as pesquisas mais relevantes.
...
Contudo, nenhum estudo combina, de maneira integrada, compressão multissinal adaptativa e priorização clínica em ciclo fechado completo. Com base na análise, as principais lacunas identificadas são:
...
\label{cap:modelo}
O modelo ViSPAC foi projetado para responder à questão de pesquisa principal formulada na Seção~\ref{sec:qp}, que questiona como aplicar priorização dinâmica com base em escores de múltiplos sinais vitais para otimizar a compressão, a coleta e a transmissão de dados clínicos em ambiente Edge-Fog-Cloud. As lacunas identificadas nos trabalhos relacionados, a ausência de compressão adaptativa multivariada e o uso restrito de compressão híbrida para apenas um sinal, motivaram diretamente as decisões de projeto descritas a seguir.
...
Diferentes sinais vitais têm impactos distintos no escore NEWS2: uma alteração sutil na $SpO_2$ (como a queda de 96\% para 92\%) adiciona 2 pontos, enquanto a frequência cardíaca (FC) exige uma variação muito mais ampla (de 80 para 109 bpm) para somar 1 ponto \cite{RoyalCollege2017}. Além disso, enquanto a FC pode ser monitorada batimento a batimento, os sensores de \textit{SpO\textsubscript{2}} em relógios e pulseiras calculam médias de 8 a 12~s, o que reduz o ruído, mas atrasa a detecção de quedas reais na saturação \cite{Desai2015}. Clinicamente, aceitar um erro de até 5~bpm na FC raramente compromete a análise \cite{REDDY2025}, mas um desvio de 3--4% na $SpO_2$ pode mascarar o início de um quadro severo.

Para respeitar essas características fisiológicas e evitar que uma tolerância genérica comprometa a detecção de eventos, o algoritmo SDT estabelece margens de erro ($DC$ ou $\varepsilon$) individualizadas por sinal e por nível de risco. Conforme o Algoritmo~1, a FC admite variações de $\pm$2~bpm (risco ALTO) a $\pm$10~bpm (risco MÍNIMO). Já a $SpO_2$, por sua maior sensibilidade, recebe limiares mais rígidos: de $\pm$1% (riscos ALTO e MODERADO) a $\pm$3% (risco MÍNIMO). Por construção, o SDT fornece uma garantia formal de que nenhum ponto comprimido desviará do valor real além desse teto pré-configurado.
...
\begin{enumerate}
    \item \textbf{BIDMC PPG and Respiration Dataset \cite{Pimentel2016}\footnote{\url{https://physionet.org/content/bidmc/1.0.0/}}:} Disponibilizado no PhysioNet, este conjunto contém dados fisiológicos coletados de pacientes internados em UTI no \textit{Beth Israel Deaconess Medical Center} (Boston, EUA).
    \begin{itemize}
...
Abaixo são detalhados os componentes de cada região, conforme ilustrado na Figura \ref{fig:arquitetura_aws_multiregiao}. É importante ressaltar que o sistema operacional utilizado foi o Ubuntu 24.04, um sistema sólido em sua última versão estável. Possui também versões ARM, o que pode ser utilizado em cenários reais de IoT.
...
\begin{itemize}
    \item \textbf{Camada Edge (Sub-rede Edge):} Composta por 20 instâncias EC2 de categoria base \textit{t2.small} (1 vCPU, 1GB RAM). Um aspecto metodológico fundamental foi a imposição de restrições de hardware nestas instâncias para simular dispositivos IoT de baixo custo. Por meio de mecanismos do sistema operacional (Cgroups), a memória RAM foi limitada a \textbf{256 MB} e a utilização da CPU foi restringida a \textbf{50\%} da capacidade de um núcleo. Conforme mostrado na figura, estas instâncias foram configuradas com diferentes densidades de pacientes (ex.: nó E-01 simulando 5 pacientes de baixo risco) para criar cargas de trabalho heterogêneas. A comunicação de saída para a camada \textit{Fog} é realizada exclusivamente pelo protocolo MQTT.
...
O software do sistema foi implementado em Python, estruturado de forma modular para operar de forma distribuída. A escolha pela linguagem se deu pelo forte conhecimento do autor nessa linguagem, além da reutilização de componentes já implementados no VSAC, na mesma linguagem. A lógica de processamento divide-se entre a execução leve na borda (centrada na redução de dados) e a execução analítica na camada de \textit{fog} (centrada na tomada de decisão clínica).
...
\begin{enumerate}
    \item \textbf{Compressão Primária (Lossy):} Todos os sinais vitais brutos (FC e $SpO_2$) são submetidos ao algoritmo SDT. Este algoritmo foi selecionado por sua complexidade computacional linear ($O(n)$), o que o torna ideal para o hardware restrito simulado. A agressividade da compressão é determinada pelo parâmetro $DC$ (Desvio de Compressão), ajustado dinamicamente.
    
    \item \textbf{Compressão Secundária (Lossless):} A saída do SDT pode passar por um segundo estágio de compressão sem perdas (Huffman ou LZW) para maximizar a redução do tamanho do pacote. No entanto, implementou-se uma regra de \textbf{\textit{bypass} para risco crítico}: caso o paciente esteja classificado em risco ALTO, o segundo estágio é ignorado e os dados são transmitidos imediatamente após o SDT. Esta estratégia visa eliminar o \textit{overhead} computacional da segunda compressão, reduzindo a latência ponta-a-ponta em detrimento de uma pequena economia de banda.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{SDT:} Baseado na implementação de SDT, disponível em repositório público\footnote{\url{https://github.com/chelaxe/SwingingDoor}}, adaptada para suportar janelas de tempo variáveis ($T_{\text{SDT}}$).
...
Cassel et al. \cite{CASSEL2024}     & -   & Sim & - \\ \hline
Chang et al. \cite{Chang2024}       & Sim & -   & PRD, MSE \\ \hline
Hassan et al. \cite{Hassan2024}     & Sim & -   & PSNR, PRD, MSE \\ \hline
Vakil et al. \cite{Vakil2024}       & Sim & Sim\textsuperscript{a} & - \\ \hline
...
\textsuperscript{a} Vakil et al. reportam economia de energia, métrica correlata à eficiência de transmissão.
...
\begin{enumerate}
    ...
    \[
    T_C = \left( 1 - \frac{S_f}{S_i} \right) \times 100\%
    \]
    Quanto maior o valor de $T_C$, maior a eficiência do sistema em economizar largura de banda.
...
Configurado no Terraform como \texttt{scenario1\_baseline}. Este cenário representa a abordagem tradicional de monitoramento contínuo sem inteligência na borda.
\begin{itemize}
    \item \textbf{Configuração:} Módulos de compressão desativados. Encaminhamento \textit{fog–cloud} em modo síncrono.
...
Configurado no Terraform como \texttt{scenario2\_static}. Avalia o ganho obtido puramente pela introdução de algoritmos de compressão, sem sensibilidade ao contexto clínico. Este cenário se equipara ao VSAC.
\begin{itemize}
    \item \textbf{Configuração:} Algoritmos SDT (Lossy) e Huffman/LZW (Lossless) ativados. Encaminhamento \textit{fog–cloud} em modo síncrono.
...
Por fim, é importante ressaltar que, ao longo do desenvolvimento, os três cenários foram executados múltiplas vezes antes da coleta final dos resultados. Essas execuções sucessivas permitiram a identificação e a correção de pequenos defeitos de implementação (\textit{bugs}), além de servirem como verificação de estabilidade dos resultados. Em todas as execuções, as métricas de desempenho mantiveram-se consistentes com os valores reportados neste capítulo, variando dentro de margens marginais atribuíveis à variabilidade natural da infraestrutura de nuvem (e.g., escalonamento de CPU e latência inter-regional). Os resultados apresentados correspondem à execução final, conduzida após a estabilização completa do código.
...
Essa redução expressiva de tráfego é atribuída à atuação do \textbf{Algoritmo de Back-off (Algoritmo 2)} e à adaptação ao risco do paciente. Ao identificar períodos de estabilidade clínica e desacoplar a coleta da transmissão, o ViSPAC permite que o dispositivo de borda permaneça em modos de baixo consumo por períodos prolongados, despertando apenas para verificações locais (Algoritmo 3) ou quando o intervalo de envio é atingido. Diferentemente do Cenário Estático, que opera com janela fixa, o ViSPAC compra tempo de silêncio na rede, custeado pela estabilidade do paciente.
...
Os resultados indicam ainda uma oportunidade a ser explorada em cenários futuros. Pacientes classificados em risco mínimo, cujos sinais permanecem estáveis por períodos prolongados, poderiam tolerar distorções maiores que as atuais sem comprometimento clínico, visto que as variações comprimidas são irrelevantes do ponto de vista diagnóstico. Aumentar o $DC$ nesses casos permitiria reduzir ainda mais o volume de dados transmitidos e o consumo de energia. A investigação do limiar máximo aceitável de distorção por faixa de risco, com validação por profissionais de saúde, constitui uma direção promissora para trabalhos futuros.
...
Rede + processamento edge & 1.125,9 ms & 1.061,8 ms & 1.048,0 ms \\ \hline
...
Para validar a hipótese de que o sistema reaja corretamente a diferentes perfis clínicos, foi analisado o comportamento temporal de três pacientes distintos durante a simulação.
...
Inicialmente, o paciente apresentava risco MODERADO. Porém, logo após o início da simulação, o paciente teve seu risco alterado para BAIXO e depois novamente para MODERADO, o que levou o ViSPAC a ajustar o tempo de coleta e os parâmetros de compressão. Ao longo da jornada, o paciente oscilou entre riscos moderados e baixos, atingindo, inclusive, o risco ALTO em alguns momentos.
...
Para avaliar a pertinência clínica das decisões de projeto do ViSPAC, realizou-se uma consulta a uma profissional da área da saúde. A respondente possui formação em Enfermagem, com títulos de Mestre e Doutora, além de especializações em Saúde Pública, Enfermagem do Trabalho e Auditoria de Serviços de Saúde. Atua como monitora de simulação clínica em ambiente hospitalar simulado, utilizando protocolos como o NEWS2 e o MEWS em cenários de treinamento com pacientes adultos.

A consulta abordou seis dimensões relevantes para o modelo proposto, cujas respostas são sintetizadas a seguir.
\begin{itemize}

\item \textbf{Escolha do escore clínico.} A especialista declarou utilizar o \textbf{NEWS2} e o MEWS em sua prática profissional, confirmando que o escore adotado pelo ViSPAC é reconhecido e aplicado em contextos clínicos reais. Seu nível de confiança na avaliação por escores foi de 6 em uma escala de 1 a 7 (propositalmente sem possuir o 10 para não induzir respostas comuns), indicando elevada confiança no instrumento, embora reconheça que escores não substituem a avaliação clínica completa.
...
\item \textbf{Aceitabilidade da latência.} A latência de aproximadamente 1,05~s do ciclo de \textit{feedback} do ViSPAC foi apresentada como referência (arredondada para 1,5~s na formulação da pergunta, para avaliar um limiar mais conservador). A especialista considerou esse tempo "perfeitamente adequado para monitoramento não invasivo", corroborando a adequação da arquitetura proposta para cenários de vigilância remota.

\item \textbf{Confiança em sistemas automatizados.} A respondente afirmou que utilizaria um sistema de alerta automático "para apoiar a tomada de decisão, mas nunca como substituto da avaliação clínica", pois "a tecnologia amplia nossa capacidade de vigilância e antecipa riscos", embora não capte "nuances, como queixas subjetivas e contexto social". Essa perspectiva reforça o posicionamento do ViSPAC como um \textbf{sistema de auxílio à tomada de decisão clínica}: ao fornecer dados priorizados e alertas baseados no NEWS2, o modelo amplia a capacidade de vigilância do profissional sem pretender substituir o julgamento clínico, que incorpora variáveis qualitativas inacessíveis a sensores remotos.

\item \textbf{Sensibilidade versus especificidade.} Diante da escolha entre receber muitos alertas (com falsos positivos) ou poucos (com risco de omissão), a especialista optou pela \textbf{maior sensibilidade} (mais alertas), ressaltando a necessidade de uma "estratégia para evitar a fadiga de alarmes". O mecanismo de vigilância contínua do ViSPAC (Algoritmo~3) opera nessa direção: mantém a sensibilidade de detecção mesmo durante longos intervalos de \textit{back-off}, enquanto a classificação por nível de risco nas filas de prioridade organiza os alertas para reduzir a sobrecarga informacional.

\item \textbf{Critério para intervenção remota.} A especialista indicou que enviaria uma ambulância com base exclusivamente em dados remotos apenas "quando percebesse evidência forte de risco iminente à vida e impossibilidade de contato", o que reforça a importância da classificação por níveis de risco implementada pelo ViSPAC: o sistema prioriza a transmissão de alta fidelidade precisamente nos casos em que o escore NEWS2 indica deterioração grave.
\end{itemize}

Em síntese, as respostas da especialista corroboram as principais decisões arquiteturais do modelo: a adoção do NEWS2 como escore de referência, a seleção de FC e $SpO_2$ como sinais prioritários, a adequação da latência obtida para monitoramento não invasivo e o equilíbrio entre a sensibilidade de detecção e a eficiência de recursos. A consulta, embora limitada a uma única respondente, fornece uma perspectiva clínica que complementa a avaliação quantitativa apresentada nas seções anteriores.

\subsection{Comparação com Trabalhos Relacionados}
A Tabela~\ref{tab:comparacao_trs} apresenta uma comparação quantitativa entre os resultados do ViSPAC e os valores relatados nos trabalhos relacionados. Essa análise permite posicionar a contribuição desta dissertação no contexto do estado da arte. É importante ressaltar que nem todas as métricas são diretamente comparáveis entre os estudos, uma vez que diferem em domínio de aplicação (ECG vs.\ sinais vitais escalares), em definição estatística (média global vs.\ percentil) e em natureza da grandeza medida (redução de transmissões vs.\ economia energética). A coluna \textit{Observações} da tabela explicita essas diferenças para cada trabalho, e a discussão a seguir as considera na interpretação dos resultados.

Dos dez trabalhos relacionados analisados no Capítulo~3, três foram excluídos da comparação quantitativa por incompatibilidade de domínio ou de métricas.~\cite{Zhou2025} investigam o impacto de codecs de vídeo sobre sinais de fotopletismografia remota (rPPG/BVP), utilizando métricas orientadas a vídeo (PSNR, SSIM) que não possuem equivalência direta com a compressão de séries temporais escalares adotada no ViSPAC.~\cite{MOHAPATRA2025} propõem um sistema de monitoramento remoto com fusão de sensores, porém não implementam compressão de dados nem reportam métricas de distorção ou de taxa de compressão, inviabilizando uma comparação numérica.~\cite{TAO2025} focam em criptografia baseada em estrutura Feistel para redes de sensores médicos; embora incluam compressão por regressão e priorização por importância do sinal, o estudo não quantifica a distorção introduzida pela compressão nem reporta taxa de compressão em termos percentuais, impossibilitando a comparação direta.
...
\footnotesize
\begin{tabular}{|p{2.8cm}|p{2cm}|p{2.5cm}|p{3cm}|p{3.5cm}|}
\hline
\textbf{Trabalho} & \textbf{Compressão} & \textbf{Distorção / Qualidade} & \textbf{Latência / Eficiência} & \textbf{Observações} \\ \hline
ViSPAC (proposto) & 81,6\% & PRD médio global 1,16\% & $T_{loop}$ 1,05\,s; 96,7\% redução transmissões & Média global; compressão absoluta \\ \hline
VSAC \cite{Andrade2025} & 46\%\textsuperscript{a} & PRD $<$ 36\% & -- & \textsuperscript{a}Ganho relativo sobre métodos isolados, não taxa absoluta \\ \hline
Esmaeili et al. \cite{ESMAEILI2023} & --\textsuperscript{b} & -- & 7\% economia energética & \textsuperscript{b}Compressão adaptativa via \textit{compressive sensing}; taxa não reportada diretamente \\ \hline
Chang et al. \cite{Chang2024} & 80\% & 0,8 erro/402 casos & -- & Apenas ECG; taxa absoluta \\ \hline
Hassan et al. \cite{Hassan2024} & 93,6\% & PRD 2\% & 91,35\% redução tráfego & Apenas ECG; \textit{compressive sensing} \\ \hline
Vakil et al. \cite{Vakil2024} & 98,6\% & -- & 98\% economia energia & Economia de energia, não redução de transmissões \\ \hline
Cassel et al. \cite{CASSEL2024} & -- & -- & 5,3\,s (P60 críticos) & Percentil 60 de sinais críticos, não média global \\ \hline
Shankani et al. \cite{Shankani2025} & -- & -- & 40\% redução tráfego; 64\% energia & Simulação MATLAB; sem compressão \\ \hline
\end{tabular}
\fonte{Elaborado pelo autor.}
\end{table}
...
\item\textbf{Taxa de Compressão.} O ViSPAC alcança 81,6\% de compressão média, valor comparável aos 80\% (5:1) de Chang et al.~\cite{Chang2024} e significativamente superior ao VSAC~\cite{Andrade2025}. Cabe notar que o valor de 46\% reportado pelo VSAC refere-se ao ganho relativo sobre métodos de compressão isolados (lossy ou lossless), e não a uma taxa de compressão absoluta, o que torna a comparação direta com os demais trabalhos apenas indicativa. Trabalhos como Hassan et al.~\cite{Hassan2024} (15,7:1) e Vakil et al.~\cite{Vakil2024} (70:1) reportam taxas superiores; contudo, esses estudos focam exclusivamente em ECG, com métodos de \textit{compressive sensing} ou de predição linear, não avaliando múltiplos sinais vitais nem a adaptação dinâmica. A vantagem do ViSPAC reside na \textbf{seletividade clínica}: a compressão agressiva é aplicada apenas a pacientes estáveis, enquanto pacientes críticos recebem transmissão de alta fidelidade.

\item\textbf{Distorção (PRD).} O PRD médio global de 1,16\% do ViSPAC é consideravelmente inferior aos 2\% reportados por Hassan et al.~\cite{Hassan2024}, mesmo com taxas de compressão comparáveis. Essa diferença decorre da estratégia adaptativa: ao concentrar a perda de informação nos períodos de estabilidade clínica e preservar integralmente os dados em momentos críticos, o modelo minimiza a distorção média global sem sacrificar a capacidade diagnóstica. Esmaeili et al.~\cite{ESMAEILI2023}, embora empreguem compressão adaptativa via \textit{compressive sensing}, não reportam taxa de compressão nem distorção em termos percentuais, limitando a comparação a aspectos qualitativos, como a economia energética de 7\% observada em simulações com quatro sensores.

\item\textbf{Latência e Responsividade.} A latência de ciclo de 1.048,9~ms posiciona o ViSPAC favoravelmente em relação aos 5,3~segundos de Cassel et al.~\cite{CASSEL2024}. Contudo, as métricas não são diretamente comparáveis: Cassel reporta o percentil 60 do tempo de processamento para sinais classificados como muito críticos, enquanto o ViSPAC reporta a média global de todos os pacientes, independentemente do nível de risco. Ainda assim, a melhoria relativa é atribuída a três fatores: o processamento primário do NEWS2 na camada \textit{fog} (em vez de depender exclusivamente da \textit{cloud}), a leveza do protocolo MQTT e o encaminhamento assíncrono para a nuvem, que remove a espera pela confirmação da nuvem do caminho crítico.

\item\textbf{Eficiência de Transmissão.} A redução de 96,7\% no número de transmissões supera os 40\% de Shankani et al.~\cite{Shankani2025} e os 91,35\% de Hassan et al.~\cite{Hassan2024}. Vakil et al.~\cite{Vakil2024} reportam 98\% de economia energética. Vale notar que redução de transmissões e economia de energia são métricas correlatas (menos transmissões implicam menor uso do rádio), porém não equivalentes, pois o consumo energético depende também de fatores como o processamento local e o modo de \textit{sleep} do dispositivo. O ViSPAC oferece uma vantagem qualitativa adicional não contemplada nos demais trabalhos: a garantia de que pacientes em deterioração clínica recebem monitoramento intensivo imediato, sem que a economia de recursos comprometa a vigilância dos casos críticos.
\end{itemize}
...
\textbf{Limitações:}
\begin{itemize}
\item \textbf{Ambiente simulado:} A validação foi conduzida em ambiente de nuvem com restrições artificiais de hardware, não reproduzindo todas as condições de um ambiente IoT real (interferências, variabilidade de bateria, etc.). Também, não foi avaliado o custo total de operação em uma implantação real, considerando instâncias de computação, dispositivos vestíveis e conectividade. Em cenários de larga escala, como os de Cidades Inteligentes, o custo de manter a infraestrutura de \textit{fog} e \textit{cloud} pode ser um fator limitante. Uma análise econômica detalhada é necessária para avaliar a viabilidade financeira do modelo antes de qualquer transferência para produção.
...
\item \textbf{Dependência de políticas públicas de saúde:} A adoção em larga escala do monitoramento remoto de pacientes conforme proposto pelo ViSPAC pressupõe que essa prática seja tratada como uma política pública de saúde. Isso exigirá investimentos em infraestrutura de conectividade, em capacitação de profissionais e em dispositivos, além de discussões sobre regulamentação, responsabilidades clínicas e privacidade dos dados. Sem esse debate institucional, a transferência da tecnologia para o ambiente real permanece limitada.
\end{itemize}
...
A principal contribuição desta dissertação é a integração, em ciclo fechado, de compressão adaptativa multissinal e priorização clínica dinâmica baseada no escore NEWS2 dentro de uma arquitetura Edge-Fog-Cloud. Esse ciclo fechado de \textit{feedback}, ausente nos trabalhos relacionados analisados, permite que o estado clínico do paciente governe, em tempo hábil, os parâmetros de compressão e a frequência de coleta, equilibrando eficiência de rede e segurança clínica.
...
A arquitetura e os algoritmos desenvolvidos nesta dissertação têm potencial direto de aplicação no mercado de saúde digital, transcendendo o ambiente acadêmico. A transferência dessa tecnologia para a sociedade pode ser visualizada em três frentes de produtos complementares, cobrindo toda a linha de atendimento, conforme ilustrado na Figura~\ref{fig:transferencia_tecnologia} e que poderia ser testada em um ambiente de cidade inteligente (como São Leopoldo).
